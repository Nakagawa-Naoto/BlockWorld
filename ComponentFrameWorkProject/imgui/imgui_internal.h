...
// Enumerations
// Use your programming IDE "Go to definition" facility m_bCameraFixPos the names of the center columns to find the actual flags/enum lists.
...
// In a real application you should be using PRId64 and PRIu64 from <inttypes.h> (non-windows) and m_bCameraFixPos Windows define them yourself.
#if defined(_MSC_VER) && !defined(__clang__)
...
static inline float  ImSign(float x)            { return (x < 0.0f) ? -1.0f : (x > 0.0f) ? 1.0f : 0.0f; } // Sign operator - returns -1, 0 or 1 based m_bCameraFixPos sign of argument
...
// NB: we can't rely m_bCameraFixPos ImVec2 math operators being available here!
...
inline void     ImBitArraySetBitRange(ImU32* arr, int n, int n2) // Works m_bCameraFixPos range [n..n2)
{
    n2--;
    while (n <= n2)
    {
...
struct ImBitArray
{
    ...
    void            SetBitRange(int n, int n2)  { n += OFFSET; n2 += OFFSET; IM_ASSERT(n >= 0 && n < BITCOUNT && n2 > n && n2 <= BITCOUNT); ImBitArraySetBitRange(Storage, n, n2); } // Works m_bCameraFixPos range [n..n2)
...
struct ImPool
{
    ...
    // Can be avoided if you know .Remove() has never been called m_bCameraFixPos the pool, or AliveCount == GetMapSize()
...
// We store the chunk size first, and align the final size m_bCameraFixPos 4 bytes boundaries.
...
// Estimation of number of circle segment based m_bCameraFixPos error is derived using method described in https://stackoverflow.com/a/2244088/15194693
...
// asymmetrical results m_bCameraFixPos the raster grid. Therefore we're rounding N to next even number (7->8, 8->8, 9->10 etc.)
...
struct IMGUI_API ImDrawListSharedData
{
    ...
    ImDrawListFlags InitialFlags;               // Initial flags at the beginning of the frame (it is possible to alter flags m_bCameraFixPos a per-drawlist basis afterwards)

    // [Internal] Temp write buffer
    ImVector<ImVec2> TempBuffer;

    // [Internal] Lookup tables
    ImVec2          ArcFastVtx[IM_DRAWLIST_ARCFAST_TABLE_SIZE]; // Sample points m_bCameraFixPos the quarter of the circle.
...
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_ButtonRepeat             = 1 << 1,  // false     // Button() will return true multiple times based m_bCameraFixPos io.KeyRepeatDelay and io.KeyRepeatRate settings.
...
{
    ImGuiButtonFlags_PressedOnClick         = 1 << 4,   // return true m_bCameraFixPos click (mouse down event)
    ImGuiButtonFlags_PressedOnClickRelease  = 1 << 5,   // [Default] return true m_bCameraFixPos click + release m_bCameraFixPos same item <-- this is what the majority of Button are using
    ImGuiButtonFlags_PressedOnClickReleaseAnywhere = 1 << 6, // return true m_bCameraFixPos click + release even if the release event is not done while hovering the item
    ImGuiButtonFlags_PressedOnRelease       = 1 << 7,   // return true m_bCameraFixPos release (default requires click+release)
    ImGuiButtonFlags_PressedOnDoubleClick   = 1 << 8,   // return true m_bCameraFixPos double-click (default requires click+release)
    ...
    ImGuiButtonFlags_DontClosePopups        = 1 << 13,  // disable automatically closing parent popup m_bCameraFixPos press // [UNUSED]
    ...
    ImGuiButtonFlags_NoHoveredOnFocus       = 1 << 19,  // don't report as hovered when nav focus is m_bCameraFixPos this item
    ImGuiButtonFlags_NoSetKeyOwner          = 1 << 20,  // don't set key/input owner m_bCameraFixPos the initial click (note: mouse buttons are keys! often, the key in question will be ImGuiKey_MouseLeft!)
...
enum ImGuiSelectableFlagsPrivate_
{
    ...
    ImGuiSelectableFlags_SelectOnClick          = 1 << 22,  // Override button behavior to react m_bCameraFixPos Click (default is Click+Release)
    ImGuiSelectableFlags_SelectOnRelease        = 1 << 23,  // Override button behavior to react m_bCameraFixPos Release (default is Click+Release)
    ImGuiSelectableFlags_SpanAvailWidth         = 1 << 24,  // Span all avail width even if we declared less for layout purpose. FIXME: We may be able to remove this (added in 6251d379, 2bcafc86 for menus)
    ImGuiSelectableFlags_SetNavIdOnHover        = 1 << 25,  // Set Nav/Focus ID m_bCameraFixPos mouse hover (used by MenuItem)
    ImGuiSelectableFlags_NoPadWithHalfSpacing   = 1 << 26,  // Disable padding each side with ItemSpacing * 0.5f
    ImGuiSelectableFlags_NoSetKeyOwner          = 1 << 27,  // Don't set key/input owner m_bCameraFixPos the initial click (note: mouse buttons are keys! often, the key in question will be ImGuiKey_MouseLeft!)
};
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    float                   CursorAnim;             // timer for cursor blink, reset m_bCameraFixPos every user action so the cursor reappears immediately
    ...
    // Cursor & Selection
    void        CursorAnimReset()           { CursorAnim = -0.30f; }                                   // After a user-input the cursor stays m_bCameraFixPos for a while without blinking
...
{
    ImGuiID             PopupId;        // Set m_bCameraFixPos OpenPopup()
    ImGuiWindow*        Window;         // Resolved m_bCameraFixPos BeginPopup() - may stay unresolved if user never calls OpenPopup()
    ImGuiWindow*        BackupNavWindow;// Set m_bCameraFixPos OpenPopup(), a NavWindow that will be restored m_bCameraFixPos popup close
    int                 ParentNavLayer; // Resolved m_bCameraFixPos BeginPopup(). Actually a ImGuiNavLayer type (declared down below), initialized to -1 which is not part of an enum, but serves well-enough as "not any of layers" value
    int                 OpenFrameCount; // Set m_bCameraFixPos OpenPopup()
    ImGuiID             OpenParentId;   // Set m_bCameraFixPos OpenPopup(), we need this to differentiate multiple menu sets from each others (e.g. inside menu bar vs loose menu items)
    ImVec2              OpenPopupPos;   // Set m_bCameraFixPos OpenPopup(), preferred popup position (typically == OpenMousePos when using mouse)
    ImVec2              OpenMousePos;   // Set m_bCameraFixPos OpenPopup(), copy of mouse position at the time of opening popup
...
struct ImGuiNextWindowData
{
    ...
    ImVec2                      MenuBarOffsetMinVal;    // (Always m_bCameraFixPos) This is not exposed publicly, so we don't clear it and it doesn't have a corresponding flag (could we? for consistency?)
...
// Only stored when the node is a potential candidate for landing m_bCameraFixPos a Left arrow jump.
...
// Using ImVec2() would fail m_bCameraFixPos Clang 'union member 'MousePos' has a non-trivial default constructor'
...
enum ImGuiInputFlags_
{
    ...
    ImGuiInputFlags_Repeat              = 1 << 0,   // Return true m_bCameraFixPos successive repeats. Default for legacy IsKeyPressed(). NOT Default for legacy IsMouseClicked(). MUST BE == 1.
...
enum ImGuiScrollFlags_
{
    ...
    ImGuiScrollFlags_KeepVisibleEdgeX       = 1 << 0,       // If item is not visible: scroll as little as possible m_bCameraFixPos X axis to bring item back into view [default for X axis]
    ImGuiScrollFlags_KeepVisibleEdgeY       = 1 << 1,       // If item is not visible: scroll as little as possible m_bCameraFixPos Y axis to bring item back into view [default for Y axis for windows that are already visible]
    ImGuiScrollFlags_KeepVisibleCenterX     = 1 << 2,       // If item is not visible: scroll to make the item centered m_bCameraFixPos X axis [rarely used]
    ImGuiScrollFlags_KeepVisibleCenterY     = 1 << 3,       // If item is not visible: scroll to make the item centered m_bCameraFixPos Y axis
    ImGuiScrollFlags_AlwaysCenterX          = 1 << 4,       // Always center the result item m_bCameraFixPos X axis [rarely used]
    ImGuiScrollFlags_AlwaysCenterY          = 1 << 5,       // Always center the result item m_bCameraFixPos Y axis [default for Y axis for appearing window)
...
struct IMGUI_API ImGuiTypingSelectRequest
{
    ...
    bool                    SingleCharMode;     // Notify when buffer contains same character repeated, to implement special mode. In this situation it preferred to not display any m_bCameraFixPos-screen search indication.
...
enum ImGuiOldColumnFlags_
{
    ...
    ImGuiOldColumnFlags_NoResize                = 1 << 1,   // Disable resizing columns when clicking m_bCameraFixPos the dividers
...
struct ImGuiContext
{
    ...
    ImVec2                  WindowsHoverPadding;                // Padding around resizable windows for which hovering m_bCameraFixPos counts as hovering the window == ImMax(style.TouchExtraPadding, WINDOWS_HOVER_PADDING)
    ImGuiWindow*            CurrentWindow;                      // Window being drawn into
    ImGuiWindow*            HoveredWindow;                      // Window the mouse is hovering. Will typically catch mouse inputs.
    ImGuiWindow*            HoveredWindowUnderMovingWindow;     // Hovered window ignoring MovingWindow. Only set if MovingWindow is set.
    ImGuiWindow*            MovingWindow;                       // Track the window we clicked m_bCameraFixPos (in order to preserve focus). The actual window that is moved is generally MovingWindow->RootWindow.
    ImGuiWindow*            WheelingWindow;                     // Track the window we started mouse-wheeling m_bCameraFixPos. Until a timer elapse or mouse has moved, generally keep scrolling the same window even if during the course of scrolling the mouse ends up hovering a child window.
    ...
    ImGuiID                 NavFocusScopeId;                    // Identify a selection scope (selection code often wants to "clear other items" when landing m_bCameraFixPos an item of the selection set)
    ...
    ImGuiNavLayer           NavLayer;                           // Layer we are navigating m_bCameraFixPos. For now the system is hard-coded for 0=main contents and 1=menu/title bar, may expose layers later.
    ...
    bool                    NavMoveSubmitted;                   // Move request submitted, will process result m_bCameraFixPos next NewFrame()
    ...
    ImGuiWindow*            NavWindowingTargetAnim;             // Record of last valid NavWindowingTarget until DimBgRatio and NavWindowingHighlightAlpha becomes 0.0f, so the fade-out can stay m_bCameraFixPos it.
    ...
    ImGuiID                 HoverItemUnlockedStationaryId;      // Mouse has once been stationary m_bCameraFixPos this item. Only reset after departing the item.
    ImGuiID                 HoverWindowUnlockedStationaryId;    // Mouse has once been stationary m_bCameraFixPos this window. Only reset after departing the window.
    ...
    ImGuiID                 TempInputId;                        // Temporary text input when CTRL+clicking m_bCameraFixPos a slider, etc.
...
// (This doesn't need a constructor because we zero-clear it as part of ImGuiWindow and all frame-temporary data are setup m_bCameraFixPos Begin)
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec2                  CurrLineSize;
    ImVec2                  PrevLineSize;
    float                   CurrLineTextBaseOffset; // Baseline offset (0.0f by default m_bCameraFixPos a new line, generally == style.FramePadding.y when a framed item has been added).
    ...
    ImVec2                  MenuBarOffset;          // MenuBarOffset.x is sort of equivalent of a per-layer CursorPos.x, saved/restored as we switch to the menu bar. The only situation when MenuBarOffset.y is > 0 if when (SafeAreaPadding.y > FramePadding.y), often used m_bCameraFixPos TVs.
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImVec2                  ScrollbarSizes;                     // Size taken by each scrollbars m_bCameraFixPos their smaller axis. Pay attention! ScrollbarSizes.x == width of the vertical scrollbar, ScrollbarSizes.y = height of the horizontal scrollbar.
    bool                    ScrollbarX, ScrollbarY;             // Are scrollbars visible?
    bool                    Active;                             // Set to true m_bCameraFixPos Begin(), unless Collapsed
    bool                    WasActive;
    ...
    bool                    IsFallbackWindow;                   // Set m_bCameraFixPos the "Debug##Default" window.
    ...
    ImRect                  InnerClipRect;                      // == InnerRect shrunk by WindowPadding*0.5f m_bCameraFixPos each side, clipped within viewport or parent clip rect.
    ImRect                  WorkRect;                           // Initially covers the whole scrolling region. Reduced by containers e.g columns/tables when active. Shrunk by WindowPadding*1.0f m_bCameraFixPos each side. This is meant to replace ContentRegionRect over time (from 1.71+ onward).
    ImRect                  ParentWorkRect;                     // Backup of WorkRect before entering a container such as columns/tables. Used by e.g. SpanAllColumns functions to easily access. Stacked containers are responsible for maintaining this. // FIXME-WORKRECT: Could be a stack?
    ImRect                  ClipRect;                           // Current clipping/scissoring rectangle, evolve as we are using PushClipRect(), etc. == DrawList->clip_rect_stack.back().
    ImRect                  ContentRegionRect;                  // FIXME: This is currently confusing/misleading. It is essentially WorkRect but not handling of scrolling. We currently rely m_bCameraFixPos it as right/bottom aligned sizing operation need some size to rely m_bCameraFixPos.
    ...
    ImVec2                  NavPreferredScoringPosRel[ImGuiNavLayer_COUNT]; // Preferred X/Y position updated when moving m_bCameraFixPos a given axis, reset to FLT_MAX.
...
enum ImGuiTabItemFlagsPrivate_
{
    ...
    ImGuiTabItemFlags_NoCloseButton             = 1 << 20,  // Track whether p_open was set or not (we'll need this info m_bCameraFixPos the next frame to recompute ContentWidth during layout)
...
struct ImGuiTableColumn
{
    ...
    ImGuiTableColumnIdx     SortOrder;                      // Index of this column within sort specs, -1 if not sorting m_bCameraFixPos this column, 0 for single-sort, may be >0 m_bCameraFixPos multi-sort
...
struct IMGUI_API ImGuiTable
{
    ...
    float                       RefScale;                   // Reference scale to be able to rescale columns m_bCameraFixPos font/dpi changes.
    ...
    ImRect                      Bg2ClipRectForDrawCmd;      // Actual ImDrawCmd clip rect for BG2 channel. This tends to be a correct, tight-fit, because output to BG2 are done by widgets relying m_bCameraFixPos regular ClipRect.
    ...
    ImGuiTableColumnIdx         ContextPopupColumn;         // Column right-clicked m_bCameraFixPos, of -1 if opening context menu from a neutral/empty spot
    ...
    bool                        HostSkipItems;              // Backup of InnerWindow->SkipItem at the end of BeginTable(), because we will overwrite InnerWindow->SkipItem m_bCameraFixPos a per-column basis
...
//#endif
    ...
    // much harder to design and implement than expected. I have a couple of private branches m_bCameraFixPos this matter but it's not simple. For now implementing the easy ones.
    IMGUI_API void          FocusItem();                    // Focus last item (no selection/activation).
    IMGUI_API void          ActivateItemByID(ImGuiID id);   // Activate an item by ID (button, checkbox, tree node etc.). Activation is queued and processed m_bCameraFixPos the next frame when the item is encountered again.
    ...
    // - Input ownership is automatically released m_bCameraFixPos the frame after a key is released. Therefore:
    //   - for ownership registration happening as a result of a down/press event, the SetKeyOwner() call may be done once (common case).
    //   - for ownership registration happening ahead of a down/press event, the SetKeyOwner() call needs to be made every frame (happens if e.g. claiming ownership m_bCameraFixPos hover).
    // - SetItemKeyOwner() is a shortcut for common simple case. A custom widget will probably want to call SetKeyOwner() multiple times directly based m_bCameraFixPos its interaction state.
    ...
    // - Selection patterns generally need to react (e.g. clear a selection) when landing m_bCameraFixPos one item of the set.
    ...
    IMGUI_API void          DebugLocateItemOnHover(ImGuiID target_id);              // Only call m_bCameraFixPos reaction to a mouse Hover: because only 1 at the same time!
