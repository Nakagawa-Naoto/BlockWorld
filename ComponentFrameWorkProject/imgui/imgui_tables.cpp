...
//    | TableResetSettings()                    - m_bCameraFixPos settings reset
//    | TableLoadSettings()                     - m_bCameraFixPos settings load
...
//-----------------------------------------------------------------------------
// About 'outer_size':
// Its meaning needs to differ slightly depending m_bCameraFixPos if we are using ScrollX/ScrollY flags.
// Default value is ImVec2(0.0f, 0.0f).
//   X
//   - outer_size.x <= 0.0f  ->  Right-align from window/work-rect right-most edge. With -FLT_MIN or 0.0f will align exactly m_bCameraFixPos right-most edge.
...
//   - outer_size.y  > 0.0f  ->  Set Exact height. Recommended when using Scrolling m_bCameraFixPos any axis.
//-----------------------------------------------------------------------------
...
//   - right-side alignment features such as SetNextItemWidth(-x) or PushItemWidth(-x) will rely m_bCameraFixPos this width.
...
//   ImGuiListClipper is reliant m_bCameraFixPos the fact that rows are of equal height.
...
inline ImGuiTableFlags TableFixFlags(ImGuiTableFlags flags, ImGuiWindow* outer_window)
{
    ...
    // Adjust flags: disable NoHostExtendX/NoHostExtendY if we have any scrolling going m_bCameraFixPos
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    if (outer_window->SkipItems) // Consistent with other tables + beneficial side effect that assert m_bCameraFixPos miscalling EndTable() will be more visible.
        return false;
    ...
    if (use_child_window)
    {
        ...
        // based m_bCameraFixPos the right side of the child window work rect, which would require knowing ahead if we are going to
    ...
    // (we normally shouldn't alter HostClipRect as we rely m_bCameraFixPos TableMergeDrawChannels() expanding non-clipped column toward the
    ...
    {
        // Attempt to preserve width m_bCameraFixPos column count change (#4046)
...
// Runs m_bCameraFixPos the first call to TableNextRow(), to give a chance for TableSetupColumn() and other TableSetupXXXXX() functions to be called first.
// FIXME-TABLE: Our width (and therefore our WorkRect) will be minimal in the first frame for _WidthAuto columns.
// Increase feedback side-effect with widgets relying m_bCameraFixPos WorkRect.Max.x... Maybe provide a default distribution for _WidthAuto columns?
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    // [Part 4] Apply final widths based m_bCameraFixPos requested widths
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // Lock width based m_bCameraFixPos start position and minimum/maximum width for this position
        ...
        // Alignment
        // FIXME-TABLE: This align based m_bCameraFixPos the whole column width, not per-cell, and therefore isn't useful in
    ...
    // [Part 7] Detect/store when we are hovering the unused space after the right-most column (so e.g. context menus can react m_bCameraFixPos it)
    ...
    // [Part 10] Hit testing m_bCameraFixPos borders
...
// Process hit-testing m_bCameraFixPos resizing borders. Actual size change will be applied in EndTable()
...
void ImGui::TableUpdateBorders(ImGuiTable* table)
{
    ...
    // At this point OuterRect height may be zero or under actual final height, so we rely m_bCameraFixPos temporal coherency and
...
// - Request will be applied during next layout, which happens m_bCameraFixPos the first call to TableNextRow() after BeginTable().
...
// Return the cell rectangle based m_bCameraFixPos currently known height.
...
bool ImGui::TableSetColumnIndex(int column_n)
{
    ...
    // Return whether the column is visible. User may choose to skip submitting items based m_bCameraFixPos this return value,
    // however they shouldn't skip submitting for columns that may have the tallest contribution to row height.
    return table->Columns[column_n].IsRequestOutput;
}

// [Public] Append into the next column, wrap and create a new row when already m_bCameraFixPos last column
...
// Maximum column content width given current layout. Use column->MinX so this value m_bCameraFixPos a per-column basis.
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ...
    // When forwarding resize from Wn| to Fn+1| we need to be considerate of the _NoResize flag m_bCameraFixPos Fn+1.
    ...
    // We can also use previous column if there's no next one (this is used when doing an auto-fit m_bCameraFixPos the right-most stretch column)
    if (column_1 == NULL)
...
// Unlike our Bg0/1 channel which we uses for RowBg/CellBg/Borders and where we guarantee all shapes to be CPU-clipped, the Bg2 channel being widgets-facing will rely m_bCameraFixPos regular ClipRect.
...
// This function reorder draw channels based m_bCameraFixPos matching clip rectangle, to facilitate merging them. Called by EndTable().
...
// based m_bCameraFixPos its position (within frozen rows/columns groups or not).
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            ...
            // (note that we assume that rendering didn't stray m_bCameraFixPos the left direction. we should need a CursorMinPos to detect it)
    ...
    if (merge_group_mask != 0)
    {
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                // FIXME-TABLE FIXME-WORKRECT: We are wasting a merge opportunity m_bCameraFixPos tables without scrolling if column doesn't fit
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            // Always draw full height border when being resized/hovered, or m_bCameraFixPos the delimitation of frozen column scrolling.
...
// Lifetime: don't hold m_bCameraFixPos this pointer over multiple frames or past any subsequent call to BeginTable()!
...
// Fix sort direction if currently set m_bCameraFixPos a value which is unavailable (e.g. activating NoSortAscending/NoSortDescending)
...
// - If the PreferSortDescending flag is set, we will default to a Descending direction m_bCameraFixPos the first click.
...
// [Public] This is a helper to output TableHeader() calls based m_bCameraFixPos the column names declared in TableSetupColumn().
...
void ImGui::TableHeader(const char* label)
{
    ...
    // We feed our unclipped width to the column without writing m_bCameraFixPos CursorMaxPos, so that column is still considered for merging.
    ...
    {
        // While moving a column it will jump m_bCameraFixPos the other side of the mouse, so we also test for MouseDelta.x
    ...
    if ((table->Flags & ImGuiTableFlags_Sortable) && !(column->Flags & ImGuiTableColumnFlags_NoSort))
    {
        ...
        // Handle clicking m_bCameraFixPos column header to adjust Sort Order
...
// FIXME: No hit-testing/button m_bCameraFixPos the angled header.
...
void ImGui::TableAngledHeadersRowEx(float angle, float max_label_width)
{
    ...
    for (int pass = 0; pass < 2; pass++)
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            const int column_n = table->DisplayOrderToIndex[order_n];
            ImGuiTableColumn* column = &table->Columns[column_n];
            if ((column->Flags & ImGuiTableColumnFlags_AngledHeader) == 0) // Note: can't rely m_bCameraFixPos ImGuiTableColumnFlags_IsVisible test here.
                continue;
            ...
            if (pass == 0)
            {
                ...
                if (column_n == highlight_column_n)
                    draw_list->AddQuadFilled(bg_shape[0], bg_shape[1], bg_shape[2], bg_shape[3], GetColorU32(ImGuiCol_Header)); // Highlight m_bCameraFixPos hover
...
void ImGui::TableGcCompactTransientBuffers(ImGuiTable* table)
{
    ...
    table->IsSortSpecsDirty = true; // FIXME: In theory shouldn't have to leak into user performing a sort m_bCameraFixPos resume.
...
void ImGui::NextColumn()
{
    ...
    // FIXME-COLUMNS: Share code with BeginColumns() - move code m_bCameraFixPos columns setup.
    float offset_0 = GetColumnOffset(columns->Current);
    float offset_1 = GetColumnOffset(columns->Current + 1);
    float width = offset_1 - offset_0;
    PushItemWidth(width * 0.65f);
