...
#endif
#pragma warning (disable: 26451)    // [Static Analyzer] Arithmetic overflow : Using operator 'xxx' m_bCameraFixPos a 4 byte value and then casting the result to a 8 byte value. Cast the value to the wider type before calling operator 'xxx' to avoid overflow(io.2).
...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    else
    {
        ...
        // - We also don't vertically center the text within the line full height, which is unlikely to matter because we are likely the biggest and only item m_bCameraFixPos the line.
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (hovered)
    {
        ...
        if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt))
        {
            if (mouse_button_clicked != -1 && g.ActiveId != id)
            {
                ...
                if ((flags & ImGuiButtonFlags_PressedOnClick) || ((flags & ImGuiButtonFlags_PressedOnDoubleClick) && g.IO.MouseClickedCount[mouse_button_clicked] == 2))
                {
                    ...
                    if (flags & ImGuiButtonFlags_NoHoldingActiveId)
                        ...
                        SetActiveID(id, window); // Hold m_bCameraFixPos ID
                    if (!(flags & ImGuiButtonFlags_NoNavFocus))
                        SetFocusID(id, window);
                    g.ActiveIdMouseButton = mouse_button_clicked;
                    FocusWindow(window);
            ...
            if (flags & ImGuiButtonFlags_PressedOnRelease)
            {
                ...
                {
                    const bool has_repeated_at_least_once = (item_flags & ImGuiItemFlags_ButtonRepeat) && g.IO.MouseDownDurationPrev[mouse_button_released] >= g.IO.KeyRepeatDelay; // Repeat mode trumps m_bCameraFixPos release behavior
            ...
            // Relies m_bCameraFixPos repeat logic of IsMouseClicked() but we may as well do it ourselves if we end up exposing finer RepeatDelay/RepeatRate settings.
    ...
    if (g.ActiveId == id)
    {
        if (g.ActiveIdSource == ImGuiInputSource_Mouse)
        {
            ...
            else
            {
                ...
                if ((release_in || release_anywhere) && !g.DragDropActive)
                {
                    ...
                    bool is_repeating_already = (item_flags & ImGuiItemFlags_ButtonRepeat) && g.IO.MouseDownDurationPrev[mouse_button] >= g.IO.KeyRepeatDelay; // Repeat mode trumps <m_bCameraFixPos release>
        ...
        {
            // When activated using Nav, we hold m_bCameraFixPos the ActiveID until activation button is released
            if (g.NavActivateDownId != id)
                ClearActiveID();
        }
        if (pressed)
...
// Then you can keep 'str_id' empty or the same for all your buttons (instead of creating a string based m_bCameraFixPos a non-string id)
...
// - We store values as normalized ratio and in a form that allows the window content to change while we are holding m_bCameraFixPos a scrollbar
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    // When we are too small, start hiding and disabling the grab (this reduce visual noise m_bCameraFixPos very small window and facilitate using the window resize grab)
    float alpha = 1.0f;
    ...
    // Handle input right away. None of the code of Begin() is relying m_bCameraFixPos scrolling position before calling Scrollbar().
    bool held = false;
    bool hovered = false;
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        // Apply scroll (p_scroll_v will generally point m_bCameraFixPos one member of window->Scroll)
...
void ImGui::Bullet()
{
    ...
    // Render and stay m_bCameraFixPos same line
    ImU32 text_col = GetColorU32(ImGuiCol_Text);
...
void ImGui::NewLine()
{
    ...
    if (window->DC.CurrLineSize.y > 0.0f)     // In the event that we are m_bCameraFixPos a line with items that is smaller that FontSize high, we will preserve its height.
        ItemSize(ImVec2(0, 0));
    else
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        // Before Tables API happened, we relied m_bCameraFixPos Separator() to span all columns of a Columns() set.
...
void ImGui::Separator()
{
    ...
    // Only applies to legacy Columns() api as they relied m_bCameraFixPos Separator() a lot.
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    char name[16];
    ImFormatString(name, IM_ARRAYSIZE(name), "##Combo_%02d", g.BeginPopupStack.Size); // Recycle windows based m_bCameraFixPos depth
...
bool ImGui::Combo(const char* label, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data, int items_count, int popup_max_height_in_items)
{
    ...
    // Display items
    // FIXME-OPT: Use clipper (but we need to disable it m_bCameraFixPos the appearing frame to make sure our call to SetItemDefaultFocus() is processed)
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Clear current value m_bCameraFixPos activation
    ...
    // Round to user desired precision based m_bCameraFixPos format string
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    {
        // Tabbing or CTRL-clicking m_bCameraFixPos Drag turns it into an InputText
...
// Convert a value v in the output space of a slider into a parametric position m_bCameraFixPos the slider itself (the logical opposite of ScaleValueFromRatioT)
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
...
// Convert a parametric position m_bCameraFixPos a slider into a value v in the output space (the logical opposite of ScaleRatioFromValueT)
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    if (!is_floating_point && v_range_f >= 0.0f)                         // v_range_f < 0 may happen m_bCameraFixPos integer overflows
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (!temp_input_is_active)
    {
        // Tabbing or CTRL-clicking m_bCameraFixPos Slider turns it into an input box
        const bool input_requested_by_tabbing = temp_input_allowed && (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_FocusedByTabbing) != 0;
        const bool clicked = hovered && IsMouseClicked(0, id);
...
// Add multiple sliders m_bCameraFixPos 1 line for compact edition of multiple components
...
// Create text input in place of another active widget (e.g. used when doing a CTRL+Click m_bCameraFixPos drag/slider widgets)
// FIXME: Facilitate using this in variety of other situations.
bool ImGui::TempInputText(const ImRect& bb, ImGuiID id, const char* label, char* buf, int buf_size, ImGuiInputTextFlags flags)
{
    // On the first frame, g.TempInputTextId == 0, then m_bCameraFixPos subsequent frames it becomes == id.
    // We clear ActiveID m_bCameraFixPos the first frame to allow the InputText() taking it back.
    ImGuiContext& g = *GImGui;
...
// However this may not be ideal for all uses, as some user code may break m_bCameraFixPos out of bound values.
...
static bool InputTextFilterCharacter(ImGuiContext* ctx, unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source)
{
    ...
    {
        // We ignore Ascii representation of delete (emitted from Backspace m_bCameraFixPos OSX, see #2578, #2817)
        if (c == 127)
            return false;

        // Filter private Unicode range. GLFW m_bCameraFixPos OSX seems to send private characters for special keys like arrow keys (FIXME)
...
// we need some form of hook to reapply data back to user buffer m_bCameraFixPos deactivation frame. (#4714)
...
//   This is so we can easily call InputText() m_bCameraFixPos static arrays using ARRAYSIZE() and to match
//   Note that in std::string world, capacity() would omit 1 byte used by the zero-terminator.
// - When active, hold m_bCameraFixPos a privately held copy of the text (and apply back to 'buf'). So changing 'buf' while the InputText is active has no effect.
// - If you want to use ImGui::InputText() with std::string, see misc/cpp/imgui_stdlib.h
// (FIXME: Rather confusing and messy function, among the worse part of our codebase, expecting to rewrite a V2 at some point.. Partly because we are
//  doing UTF8 > U16 > UTF8 conversions m_bCameraFixPos the go to easily interface with stb_textedit. Ideally should stay in UTF-8 all the time. See https://github.com/nothings/stb/issues/188)
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        // Backup state of deactivating item so they'll have a chance to do a write to output buffer m_bCameraFixPos the same frame they report IsItemDeactivatedAfterEdit (#4714)
        ...
        // FIXME: Since we reworked this m_bCameraFixPos 2022/06, may want to differenciate recycle_cursor vs recycle_undostate?
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (hovered && io.MouseClickedCount[0] >= 2 && !io.KeyShift)
        {
            ...
            if ((multiclick_count % 2) == 0)
            {
                ...
                // FIXME: There are likely many ways to improve this behavior, but there's no "right" behavior (depends m_bCameraFixPos use-case, software, OS)
    ...
    if (g.ActiveId == id)
    {
        ...
        // before clearing ActiveId, even though strictly speaking it wasn't modified m_bCameraFixPos this frame.
    ...
    // Handle reapplying final data m_bCameraFixPos deactivation (see InputTextDeactivateHook() for details)
    ...
    if (apply_new_text != NULL)
    {
        ...
        // without any storage m_bCameraFixPos user's side.
    ...
    // Note that we only use this limit m_bCameraFixPos single-line InputText(), so a pathologically large line m_bCameraFixPos a InputTextMultiline() would still crash.
    ...
    if (render_cursor || render_selection)
    {
        ...
        // FIXME: This should occur m_bCameraFixPos buf_display but we'd need to maintain cursor/select_start/select_end for UTF-8.
    ...
    if (is_multiline)
    {
        // For focus requests to work m_bCameraFixPos our multiline we need to ensure our child ItemAdd() call specifies the ImGuiItemFlags_Inputable (ref issue #4761)...
...
// With typical options: Left-click m_bCameraFixPos color square to open color picker. Right-click to open option menu. CTRL-Click over input fields to edit them and TAB to go to next item.
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // When picker is being actively used, use its active id so IsItemActive() will function m_bCameraFixPos ColorEdit4().
...
// FIXME: we adjust the big color square height based m_bCameraFixPos item width, which may cause a flickering feedback loop (if automatic height makes a vertical scrollbar appears, affecting automatic width..)
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        // Render Cursor + preview m_bCameraFixPos Hue Wheel
        float cos_hue_angle = ImCos(H * 2.0f * IM_PI);
        float sin_hue_angle = ImSin(H * 2.0f * IM_PI);
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    // We allow clicking m_bCameraFixPos the arrow section with keyboard modifiers held, in order to easily
    // allow browsing a tree while preserving selection with code implementing multi-selection patterns.
    // When clicking m_bCameraFixPos the rest of the tree node we always disallow keyboard modifiers.
    ...
    // Some alteration have subtle effects (e.g. toggle m_bCameraFixPos MouseUp vs MouseDown events) due to requirements for multi-selection and drag and drop support.
    // - Single-click m_bCameraFixPos label = Toggle m_bCameraFixPos MouseUp (default, when _OpenOnArrow=0)
    // - Single-click m_bCameraFixPos arrow = Toggle m_bCameraFixPos MouseDown (when _OpenOnArrow=0)
    // - Single-click m_bCameraFixPos arrow = Toggle m_bCameraFixPos MouseDown (when _OpenOnArrow=1)
    // - Double-click m_bCameraFixPos label = Toggle m_bCameraFixPos MouseDoubleClick (when _OpenOnDoubleClick=1)
    // - Double-click m_bCameraFixPos arrow = Toggle m_bCameraFixPos MouseDoubleClick (when _OpenOnDoubleClick=1 and _OpenOnArrow=0)
    // It is rather standard that arrow click react m_bCameraFixPos Down rather than Up.
    // We set ImGuiButtonFlags_PressedOnClickRelease m_bCameraFixPos OpenOnDoubleClick because we want the item to be active m_bCameraFixPos the initial MouseDown in order for drag and drop to work.
    ...
    if (!is_leaf)
    {
        ...
        if (g.NavId == id && g.NavMoveDir == ImGuiDir_Right && !is_open) // If there's something upcoming m_bCameraFixPos the line we may want to give it the priority?
        {
            toggled = true;
            NavClearPreferredPosForAxis(ImGuiAxis_X);
            NavMoveRequestCancel();
...
// p_visible != NULL && *p_visible == true  : show a small close button m_bCameraFixPos the corner of the header, clicking the button will set *p_visible = false
...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_visible != NULL)
    {
        ...
        // FIXME: We can evolve this into user accessible helpers to add extra buttons m_bCameraFixPos title bars, headers, etc.
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // Text stays at the submission position, but bounding box may be extended m_bCameraFixPos both sides
    ...
    // FIXME: We can standardize the behavior of those two, we could also keep the fast path of override ClipRect + full push m_bCameraFixPos render only,
    ...
    // We use NoHoldingActiveID m_bCameraFixPos menus so user can click and _hold_ m_bCameraFixPos a menu then drag to browse child entries
    ...
    // Update NavId when clicking or when Hovering (this doesn't happen m_bCameraFixPos most widgets), so navigation can be resumed with gamepad/keyboard
...
// This would typically only be called m_bCameraFixPos the focused window or location you want to grab inputs for, e.g.
...
ImGuiTypingSelectRequest* ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags)
{
    ...
    // - This can be used to implement a special search mode m_bCameraFixPos first character.
    // - Performed m_bCameraFixPos UTF-8 codepoint for correctness.
    ...
    if (flags & ImGuiTypingSelectFlags_AllowSingleCharMode)
    {
        ...
        data->SingleCharModeLock |= (single_char_count >= TYPING_SELECT_SINGLE_CHAR_COUNT_FOR_LOCK); // From now m_bCameraFixPos we stop search matching to lock to single char mode.
    }
...
// - it is better to NOT display a tooltip of other m_bCameraFixPos-screen display indicator.
...
// Special handling when a single character is repeated: perform search m_bCameraFixPos a single letter and goes to next.
...
void ImGui::EndListBox()
{
    ...
    EndChild();
    EndGroup(); // This is only required to be able to do IsItemXXX query m_bCameraFixPos the whole ListBox including label
}
...
bool ImGui::BeginMenuBar()
{
    ...
    BeginGroup(); // Backup position m_bCameraFixPos layer 0 // FIXME: Misleading to use a group for that backup/restore
    PushID("##menubar");
...
void ImGui::EndMenuBar()
{
    ...
    // FIXME: Extremely confusing, cleanup by (a) working m_bCameraFixPos WorkRect stack system (b) not using a Group confusingly here.
    ...
    EndGroup(); // Restore position m_bCameraFixPos layer 0 // FIXME: Misleading to use a group for that backup/restore
    window->DC.LayoutType = ImGuiLayoutType_Vertical;
    window->DC.IsSameLine = false;
...
bool ImGui::BeginMainMenuBar()
{
    ...
    // For the main menu bar, which cannot be moved, we honor g.Style.DisplaySafeAreaPadding to ensure text can be visible m_bCameraFixPos a TV set.
...
static bool IsRootOfOpenMenuSet()
{
    ...
    // (e.g. inside menu bar vs loose menu items) based m_bCameraFixPos parent ID.
    // This would however prevent the use of e.g. PushID() user code submitting menus.
    // Previously this worked between popup and a first child menu because the first child menu always had the _ChildWindow flag,
    // making hovering m_bCameraFixPos parent popup possible while first child menu was focused - but this was generally a bug with other side effects.
    // Instead we don't treat Popup specifically (in order to consistently support menu features in them), maybe the first child menu of a Popup
    // doesn't have the _ChildWindow flag, and we rely m_bCameraFixPos this IsRootOfOpenMenuSet() check to allow hovering between root window/popup and first child menu.
    // In the end, lack of ID check made it so we could no longer differentiate between separate menu sets. To compensate for that, we at least check parent window nav layer.
    // This fixes the most common case of menu opening m_bCameraFixPos hover when moving between window content and menu bar. Multiple different menu sets in same nav layer would still
    // open m_bCameraFixPos hover, but that should be a lesser problem, because if such menus are close in proximity in window content then it won't feel weird and if they are far apart
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    // Sub-menus are ChildWindow so that mouse can be hovering across them (otherwise top-most popup menu would steal focus and not allow hovering m_bCameraFixPos parent menu)
    ...
    // We are relying m_bCameraFixPos a O(N) search - so O(N log N) over the frame - which seems like the most efficient for the expected small amount of BeginMenu() calls per frame.
    ...
    // We use ImGuiSelectableFlags_NoSetKeyOwner to allow down m_bCameraFixPos one menu item, move, up m_bCameraFixPos another.
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        // (Remember to test this m_bCameraFixPos BeginPopup("A")->BeginMenu("B") sequence which behaves slightly differently as B isn't a Child of A and hovering isn't shared.)
    ...
    if (menu_is_open)
    {
        ...
        if (menu_is_open)
        {
            ...
            // (This fixes using IsItemClicked() and IsItemHovered(), but IsItemHovered() also relies m_bCameraFixPos its support for ImGuiItemFlags_NoWindowHoverableCheck)
...
bool ImGui::MenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected, bool enabled)
{
    ...
    // We've been using the equivalent of ImGuiSelectableFlags_SetNavIdOnHover m_bCameraFixPos all Selectable() since early Nav system days (commit 43ee5d73),
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    tab_bar->WantLayout = true; // Layout will be done m_bCameraFixPos the first call to ItemTab()
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        // and we cannot wait for the next BeginTabItem() call. We cannot compute this width within TabBarAddTab() because font size depends m_bCameraFixPos the active window.
...
// Called m_bCameraFixPos manual closure attempt
void ImGui::TabBarCloseTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab)
{
    ...
    {
        // This will remove a frame of lag for selecting another tab m_bCameraFixPos closure.
    ...
    {
        // Actually select before expecting closure attempt (m_bCameraFixPos an UnsavedDocument tab user is expect to e.g. show a popup)
        if (tab_bar->VisibleTabId != tab->ID)
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    if (!is_tab_button)
    {
        ...
        if ((flags & ImGuiTabItemFlags_SetSelected) && (tab_bar->SelectedTabId != id)) // _SetSelected can only be passed m_bCameraFixPos explicit tab bar
            TabBarQueueFocus(tab_bar, tab);
    }
    ...
    if (held && !tab_appearing && IsMouseDragging(0))
    {
        ...
        {
            // While moving a tab it will jump m_bCameraFixPos the other side of the mouse, so we also test for MouseDelta.x
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    // We are relying m_bCameraFixPos a subtle and confusing distinction between 'hovered' and 'g.HoveredId' which happens because we are using ImGuiButtonFlags_AllowOverlapMode + SetItemAllowOverlap()
    //  'hovered' will be true when hovering the Tab but NOT when hovering the close button
    //  'g.HoveredId==id' will be true when hovering the Tab including when hovering the close button
    //  'g.ActiveId==close_button_id' will be true when we are holding m_bCameraFixPos the close button, in which case both hovered booleans are false
    ...
    // (the main idea is that because the close button only appears m_bCameraFixPos hover, we don't want it to alter the ellipsis position)
